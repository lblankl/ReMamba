...
class AllHiddenMerger(HiddenMerger):
    def __init__(self, config):
...
class ConMixerModel(nn.Module):
    def __init__(
        ...
        self.conmambacfg=conmambacfg
        self.merger=LastHiddenMerger(conmambacfg) 
    ...
    def forward(self, input_ids, inference_params=None,prompt_input_idsm:Optional[torch.LongTensor] = None,
        ...
        for idx,layer in enumerate(self.layers):
            ...
            if inference :
                ...
                if idx==self.conmambacfg.mambacfg.n_layer-1:
                    hidden_states=self.merger(gist_activations,hidden_states,prompt_input_idsm)
