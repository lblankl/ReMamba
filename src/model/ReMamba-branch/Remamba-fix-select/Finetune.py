...
def main():
    ...
    # eval_datasets = load_from_disk(data_args.dataset_name).select(range(data_args.data_range, data_args.data_range+50))
    ...
    if model_args.remamba_config:
        ...
        cfg=ReMambaConfig()
    cfg.train_select_rate_max=training_args.select_rate_Max
    cfg.train_select_rate_min=training_args.select_rate_Min
    if model_args.model_name_or_path and data_args.debugging==False:
        ...
        if data_args.debugging==True:
            ...
            #init weight
            with torch.no_grad():
                model.backbone.K_proj[0].weight.data+=model.backbone.layers[0].mixer.in_proj.weight.data[:cfg.hidden_size,:]
                model.backbone.K_proj[2].weight.data+=model.backbone.layers[0].mixer.in_proj.weight.data[cfg.hidden_size:2*cfg.hidden_size,:]
                model.backbone.Q_proj[0].weight.data+=model.backbone.layers[0].mixer.in_proj.weight.data[2*cfg.hidden_size:3*cfg.hidden_size,:]
                model.backbone.Q_proj[2].weight.data+=model.backbone.layers[0].mixer.in_proj.weight.data[3*cfg.hidden_size:4*cfg.hidden_size,:]
                # model.backbone.G_proj.weight.data+=model.backbone.layers[0].mixer.in_proj.weight.data[3*cfg.hidden_size:4*cfg.hidden_size,:]
                
                model.judge.weight.data+=model.lm_head.weight.data[4:6,:]
            ...
            # if  transformer.model.embed_tokens.weight.shape[0]== vocab_size + 1:
            #     pass
            # else:
            #     transformer.resize_token_embeddings(len(transformer_tokenizer))

            # if model_args.transformer_peft:

            #     transformer=PeftModel.from_pretrained(transformer,model_args.transformer_peft)
                
            #     transformer.merge_and_unload()
            
    # mamba_tokenizer.add_special_tokens({"additional_special_tokens": ["<GIST>"]})
    # model.resize_token_embeddings(len(mamba_tokenizer))
    ...
    #         -1
    #     ] = model.backbone.embeddings.weight[:-1].mean(0)
    model.backbone.tokenizer=mamba_tokenizer
    ...
    modules_to_save=["Q_proj","V_proj","K_proj","judge","judge_norm"],
        ...
        for batch in train_dataloader:
                ...
                if steps>checkp:
                    ...
                    with accelerator.accumulate(model):
                        optimizer.zero_grad()
                        outputs = model(**batch)
                        selectloss=outputs.loss#+outputs.aux_loss
                        normalloss=outputs.normal_loss
                        judgement_loss=outputs.judgement_loss
                        loss=selectloss+normalloss+judgement_loss
                    ...
                    normalloss_acu+=normalloss.item()/accumulation_step
                    selectloss_acu+=selectloss.item()/accumulation_step
                    judgement_loss_acu+=judgement_loss.item()/accumulation_step
                    ...
                    if steps%logging_steps==0:
                        ...
                        if log_steps%100==0:
                            ...
                            accelerator.print("select-loss",selectloss_acu)
                            accelerator.print("normal-loss",normalloss_acu)
                            accelerator.print("judgement-loss",judgement_loss_acu)
                        log_steps+=1
                        
                        
                        
                        
                        accelerator.log({"total_loss":l_acu},step=log_steps)
                        accelerator.log({"select_loss":selectloss_acu},step=log_steps)
                        accelerator.log({"normal_loss":normalloss_acu},step=log_steps)
                        accelerator.log({"judgement_loss":judgement_loss_acu},step=log_steps)
                        accelerator.log({"loss":selectloss_acu},step=log_steps)
