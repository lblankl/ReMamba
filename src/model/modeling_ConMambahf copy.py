...
class MambaMixer(nn.Module):
    ...
    def slow_forward(self, input_states, cache_params: Optional[MambaCache]=None):
        ...
        for i in range(seq_len):
            g=ssm_state + deltaB_u[:, :, i, :]
            if i==seq_len-1 and seq_len>2:
                
                print("layer idex:",self.layer_idx,"gated:",g.shape)
...
class LastHiddenMerger(HiddenMerger):
    ...
    def forward(self,gist_activations,hidden_states,prompt_input_idsm,idx):
        ...
        for i in range(hidden_states.shape[0]):
            ...
            hidden_states[i][prompt_index,:]=torch.zeros_like(hidden_states[i][prompt_index,:]).to(expanded_hidden.device)
        return hidden_states
# class LastHiddenMerger(HiddenMerger):
#     def __init__(self, config):
#         super().__init__(config)
