...
class GistActivations:
    ...
    all_last_hidden_state: torch.FloatTensor
    ...
    def from_model_outputs(
        ...
        layer_num=len(model_outputs.hidden_states)
        all_last_hidden_state=torch.zeros(
            (batch_size, layer_num, num_gist_tokens, model_outputs.hidden_states[0].shape[-1]),
            dtype=model_outputs.hidden_states[0].dtype,
            device=device,
        )
        ...
        for batch_i, input_row in enumerate(input_ids):
            ...
            for idx,hidden_states in enumerate(model_outputs.hidden_states):
                all_last_hidden_state[batch_i][idx] = hidden_states[batch_i, gist_start:gist_end]
        ...
        return GistActivations(
            ...
            all_last_hidden_state=all_last_hidden_state
